...
def SudokuBoardMaker(Title, BoardName, Board, Difficulty):
    DigitReplace = [":white_large_square:", ":one:", ":two:", ":three:", ":four:", ":five:", ":six:", ":seven:", ":eight:", ":nine:"]
...
def TTTBoardMaker(Board, User1, User2, AnExtra = "\u200b"):
    ItemReplace = {"1":":one:", "2":":two:", "3":":three:", "4":":four:", "5":":five:", "6":":six:", "7":":seven:", "8":":eight:", "9":":nine:", "x":":x:", "o":":o:"}
...
def TTTWinCheck(Board):
    ...
    if (BoardSimplified[0] == "xxx") or (BoardSimplified[1] == "xxx") or (BoardSimplified[2] == "xxx") or ((BoardSimplified[0][0] == "x") and (BoardSimplified[1][0] == "x") and (BoardSimplified[2][0] == "x")) or ((BoardSimplified[0][1] == "x") and (BoardSimplified[1][1] == "x") and (BoardSimplified[2][1] == "x")) or ((BoardSimplified[0][2] == "x") and (BoardSimplified[1][2] == "x") and (BoardSimplified[2][2] == "x")) or ((BoardSimplified[0][2] == "x") and (BoardSimplified[1][1] == "x") and (BoardSimplified[2][0] == "x")) or ((BoardSimplified[0][0] == "x") and (BoardSimplified[1][1] == "x") and (BoardSimplified[2][2] == "x")):
        return  True
    elif (BoardSimplified[0] == "ooo") or (BoardSimplified[1] == "ooo") or (BoardSimplified[2] == "ooo") or ((BoardSimplified[0][0] == "o") and (BoardSimplified[1][0] == "o") and (BoardSimplified[2][0] == "o")) or ((BoardSimplified[0][1] == "o") and (BoardSimplified[1][1] == "o") and (BoardSimplified[2][1] == "o")) or ((BoardSimplified[0][2] == "o") and (BoardSimplified[1][2] == "o") and (BoardSimplified[2][2] == "o")) or ((BoardSimplified[0][2] == "o") and (BoardSimplified[1][1] == "o") and (BoardSimplified[2][0] == "o")) or ((BoardSimplified[0][0] == "o") and (BoardSimplified[1][1] == "o") and (BoardSimplified[2][2] == "o")):
        return True
    else:
        return False
...
class Games(commands.Cog):
    ...
    async def PlaySudoku(self, ctx, *args):
        def ChCHEm(RcM, RuS):
            return RuS.bot == False and RcM.message == OriginalBoard and str(RcM.emoji) in ["👁️","❌"]
        ...
        if Difficulty not in ["easy","hard","medium","random"]:
            ...
            return
        SudokuBoard = requests.get(f'https://sugoku.herokuapp.com/board?difficulty={Difficulty}')
        JSONboard = SudokuBoard.json()["board"]
        OriginalBoard = await ctx.message.channel.send(embed = SudokuBoardMaker("Sudoku", BoardName, JSONboard, Difficulty))
        ...
        try:
            ...
            if ReaEm[0].emoji == "👁️":
                await ctx.message.channel.send(embed = SudokuBoardMaker("Solution", BoardName, requests.post('https://sugoku.herokuapp.com/solve', data=SudokuBoard.content.decode("utf-8"), headers = {"Accept": "x-www-form-urlencoded"}).json()["solution"], Difficulty))
    ...
    async def PlayTTT(self, ctx):
        def ChCHanS(MSg):
            ...
            try:
                if 0 < int(MSg.content) < 10:
                    ...
                    if Table[PlaceOnBoard[0]][PlaceOnBoard[1]] != "x" and Table[PlaceOnBoard[0]][PlaceOnBoard[1]] != "o":
                        RsT = True
            except ValueError:
        ...
        if len(ctx.message.mentions) > 0:
            ...
            R = 1
            OriginalBoard = await ctx.message.channel.send(embed = TTTBoardMaker(Table, Players[0], Players[1]))
            while True:
                if R == 10:
                    ...
                    try:
                        if int(ResS.content) < 10:
                            ...
                            if TTTWinCheck(Table):
                                await OriginalBoard.edit(embed = TTTBoardMaker(Table, Players[0], Players[1], f'{Player.display_name} WINS'))
                                await ctx.message.channel.send(f'{Player.mention} Wins!! :partying_face:')
                                return
                            await OriginalBoard.edit(embed = TTTBoardMaker(Table, Players[0], Players[1]))
                    except ValueError:
