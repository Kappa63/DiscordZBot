    ...
    if TwTtype == "Retweet":
        try:
            if hasattr(TWtimeline.retweeted_status, "extended_entities"):
                TEmE.set_image(url = TWtimeline.retweeted_status.extended_entities["media"][0]["media_url_https"])
        ...
        try:
            if hasattr(TWtimeline.quoted_status, "extended_entities"):
                TEmE.set_image(url = TWtimeline.quoted_status.extended_entities["media"][0]["media_url_https"])
        ...
        if hasattr(TWtimeline, "extended_entities"):
            TEmE.set_image(url = TWtimeline.extended_entities["media"][0]["media_url_https"])
...
def ChTwTp(TWtimeline):
    if hasattr(TWtimeline, "retweeted_status") and TWtimeline.retweeted_status:
        ...
        return "Quote"
    elif hasattr(TWtimeline, "in_reply_to_status_id") and TWtimeline.in_reply_to_status_id:
        return "Comment"
    else:
        return "Tweet"
        ...
        if TWinput[0].lower() == "search" and args:
            ...
            if " ".join(TWinput):
                ...
                for TWuser in Twitter.search_users(TWinput, count = 10):
                    C += 1
                    if C == 1:
                        STEm = discord.Embed(title = f':mag: Search for "{" ".join(TWinput)}"',  description = "\u200b", color = 0x0384fc)
                    IsVerified = ""
                    if TWuser.verified:
        ...
        try:
            try:
                ...
                if TWprofile.location:
                    TEm.add_field(name = "Location: ", value = TWprofile.location, inline = True)
                if TWprofile.url:
                    TEm.add_field(name = "Website: ", value = (requests.head(TWprofile.url)).headers['Location'], inline = True)
                ...
                while True:
                    try:
                        ...
                        if ReaEm[0].emoji == "⬅️" and TwTNum == 0:
                            ...
                            TwTNum -= 1
                            await TwTsL.edit(embed = EmbedMaker(TWprofile, IsVerified, ChTwTp(TWtimeline[TwTNum]), TWtimeline[TwTNum], TwTNum, len(TWtimeline)))
                        elif ReaEm[0].emoji == "➡️" and OnMain:
                            OnMain = False    
                            TwTNum = 0
                            await TwTsL.edit(embed = EmbedMaker(TWprofile, IsVerified, ChTwTp(TWtimeline[TwTNum]), TWtimeline[TwTNum], TwTNum, len(TWtimeline)))
                        elif ReaEm[0].emoji == "➡️" and len(TWtimeline) > TwTNum+1 and TwTNum >= 0:
                            TwTNum += 1
                            await TwTsL.edit(embed = EmbedMaker(TWprofile, IsVerified, ChTwTp(TWtimeline[TwTNum]), TWtimeline[TwTNum], TwTNum, len(TWtimeline)))
