...
class HelpInfo(commands.Cog):
    ...
    async def SendS(self, ctx):
        ...
        HEm.add_field(name="zadd [word]: ", value="Adds a word/phrase to keep track of", inline=False)
        HEm.add_field(name="zremove [word]: ", value="Removes an existing word/phrase being tracked", inline=False)
        ...
        HEm.add_field(name="zclear: ", value="Clear everything, AKA remove ALL info(this is irreversable)", inline=False)
        await ctx.message.channel.send(embed=HEm)

    @SendH.command(aliases=["social", "socials", "reddit", "twitter", "youtube", "multireddit", "multireddits", "redditor", "twitch"])
    @commands.cooldown(1, 1, commands.BucketType.user)
    async def SendR(self, ctx):
        HEm = discord.Embed(title="**ZBot Social(s) Help**", description="\u200b", color=0x0AF531)
        ...
        HEm.add_field(name="zyoutube [Channel Name]: ", value="Details About the Channel and 20 of The Latest Uploads", inline=False)
        HEm.add_field(name="zyoutube search [Search Term]: ", value="Searches for 10 Channels Related to Search Term", inline=False)
        HEm.add_field(name="ztwitch [User]: ", value="Returns a twitch user's Profile", inline=False)
        HEm.add_field(name="ztwitch stream [User]: ", value="returns a twitch user's current Live stream", inline=False)
        HEm.add_field(name="ztwitch clips [User]: ", value="20 of the Channels Clips", inline=False)
    ...
    async def SendG(self, ctx):
        ...
        HEm.add_field(name="zsudoku [easy/medium/hard/random]: ", value="Returns a sudoku puzzle with the given difficulty", inline=False)
        HEm.add_field(name="zttt [mention]: ", value="Starts a Tic-Tac-Toe game vs the mentioned user", inline=False)
        HEm.add_field(name="zchess [mention]: ", value="Starts a Chess game vs the mentioned user", inline=False)
        HEm.add_field(name="zcptd: ", value="Chess Puzzle of The Day from Chess.com (For Voters and donators Only)", inline=False)
        HEm.add_field(name="zcptddaily: ", value="Returns when the next CPTD Daily will happen", inline=False)
        HEm.add_field(name="zcptddaily start: ", value="Start receiving daily CPTDs in the current channel (Tier 2 or more donators ONLY)", inline=False)
        HEm.add_field(name="zcptddaily end/stop: ", value="Stop receiving daily CPTDs (Tier 2 or more donators ONLY)", inline=False)
        HEm.add_field(name="zosu [Username]: ", value="Get the user's OSU! profile", inline=False)
    ...
    async def SendC(self, ctx):
        ...
        HEm.add_field(name="zapod: ", value="Astronomy Picture of the Day (Voters and donators ONLY)", inline=False)
        HEm.add_field(name="zapoddaily: ", value="Returns when the next APOD Daily will happen", inline=False)
        HEm.add_field(name="zapoddaily start: ", value="Start receiving daily APODs in the current channel (Tier 2 or more donators ONLY)", inline=False)
        HEm.add_field(name="zweather [Place]: ", value="Get the weather", inline=False)
        HEm.add_field(name="zgis [Search Term]: ", value="Google Image Search. 20 google image results", inline=False)
        HEm.add_field(name="zgoogle [Search Term]: ", value="Google Search. 20 google results", inline=False)
    ...
    async def SendIM(self, ctx):
        HEm = discord.Embed(title="**ZBot IMDb Help**", description="\u200b", color=0x0AF531)
        HEm.add_field(name="zimdb [Movie/Series]: ", value="Returns info about the Movie/Series", inline=False)
        HEm.add_field(name="zimdb search [Search Argument]: ", value="Searches for 10 Movies/Series related to search argument", inline=False)
        await ctx.message.channel.send(embed=HEm)
    ...
    async def SendT(self, ctx):
        ...
        HEm.add_field(name="zrule34 surf [Search Term]: ", value="Returns 100 rule34 images from a random page (Voters and donators ONLY) (NSFW)", inline=False)
        await ctx.message.channel.send(embed=HEm)
    ...
    async def SendA(self, ctx):
        ...
        HEm.add_field(name="Notes: ", value="**-Hentai: **Some hentai are not available. This is to abide by the discord TOS.", inline=False)
        await ctx.message.channel.send(embed=HEm)

    @SendH.command(aliases=["image", "images", "fry", "qr"])
    @commands.cooldown(1, 1, commands.BucketType.user)
    async def SendI(self, ctx):
        HEm = discord.Embed(title="**ZBot Image Help**", description="\u200b", color=0x0AF531)
        ...
        HEm.add_field(name="zpog: ", value="Random Pog", inline=False)
        ...
        HEm.add_field(name="zgiphy [Search argument]: ", value="Returns a RANDOM gif from top 50 results on giphy", inline=False)
    ...
    async def SendJ(self, ctx):
        HEm = discord.Embed(title="**ZBot Joke Help**", description="\u200b", color=0x0AF531)
        HEm.add_field(name="zdadjoke: ", value="Dad Jokes", inline=False)
        HEm.add_field(name="zjoke: ", value="Jokes (No sensetive material)", inline=False)
        HEm.add_field(name="zdarkjoke: ", value="Darker Jokes (Might contain sensetive material, at own descretion)", inline=False)
        HEm.add_field(name="zpun: ", value="Puns.", inline=False)
        await ctx.message.channel.send(embed=HEm)

    @SendH.command(aliases=["quotes", "quote", "insult", "insults", "advice", "kanye", "taylor", "taylorswift"])
    @commands.cooldown(1, 1, commands.BucketType.user)
    async def SendQ(self, ctx):
        ...
        #? HEm.add_field(name="zqotddaily end/stop: ", value="Stop receiving daily QOTDs (Tier 2 or more donators ONLY)", inline=False)
        HEm.add_field(name="zinsult: ", value="Returns an insult", inline=False)
        HEm.add_field(name="Aliases:", value="**-Kanye =** `kanye, kanyewest`\n\n**-Taylor =** `taylor, taylorswift`")
        await ctx.message.channel.send(embed=HEm)
    ...
    async def SendM(self, ctx):
        ...
        HEm.add_field(name="Aliases:", value="**-Binary =** `bin, binary`\n\n**-Binary Create =** `binary create, binary make`\n\n**-Coinflip =** `cf, coinflip`")
        HEm.add_field(name="Notes:", value="**-zremind:** is limited to 1day max. Could occasionally fail to notify you due to the bot going down.", inline=False)
        await ctx.message.channel.send(embed=HEm)
